 ...
 * `~/.grok/sessions/**\/updates.jsonl`, `~/.omp/agent/sessions/**\/*.jsonl`,
 * the opencode SQLite database) rather than relying on T3 Code's own
 ...
 *
 * v6 adds `omp` and `opencode` to {@link UsageProviderKind}; v5 adds `grok`;
...
export const UsageProviderKind = Schema.Literals(["claude", "codex", "grok", "omp", "opencode"]);
